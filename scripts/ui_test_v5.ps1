...
function SwipeUp {
    adb_cmd @("shell","input","swipe","540","1800","540","800","1500") | Out-Null
    Start-Sleep -Milliseconds 1500
}

function SwipeDown {
    adb_cmd @("shell","input","swipe","540","800","540","1800","1500") | Out-Null
    Start-Sleep -Milliseconds 1500
}
...
function FindEl([string]$dump, [string]$text, [int]$minY = 0, [int]$maxY = 2400) {
    $escaped = [regex]::Escape($text)
    $pat = "(text|content-desc)=`"([^`"]*[\u2022\u00b7]\s*)?${escaped}`"[^>]*bounds=`"\[(\d+),(\d+)\]\[(\d+),(\d+)\]`""
    
    $matches = [regex]::Matches($dump, $pat)
    foreach ($m in $matches) {
        $x1 = [int]$m.Groups[3].Value
        $y1 = [int]$m.Groups[4].Value
        $x2 = [int]$m.Groups[5].Value
...
function GoHome    { $script:CurrentBookNum = 0; Tap $T_HOME   $TY 1500 }
function GoRead    { Tap $T_READ   $TY 1500 }
function GoSearch  { $script:CurrentBookNum = 0; Tap $T_SEARCH $TY 1500 }
function GoCalendar{ $script:CurrentBookNum = 0; Tap $T_CAL    $TY 1500 }
function GoProfile { $script:CurrentBookNum = 0; Tap $T_PROF   $TY 1500 }
...
function EnsureBookList {
    $d = GetDump
    $hasBackButton = $d -match "(content-desc|text)=\`"[^\`"]*\u2190[^\`"]*\`""
    $isBookList = $d -match "Holy Bible" -or $d -match "Old Testament" -or $d -match "New Testament"
    
    if ($isBookList -and -not $hasBackButton) {
        return $true
    }
    
    if ($hasBackButton) {
        Write-Host "  Back button detected. Pressing back (fast path)..." -ForegroundColor Gray
        PressBack
        # Check if we are now in book list
        $d = GetDump
        $hasBackButton = $d -match "(content-desc|text)=\`"[^\`"]*\u2190[^\`"]*\`""
        $isBookList = $d -match "Holy Bible" -or $d -match "Old Testament" -or $d -match "New Testament"
        if ($isBookList -and -not $hasBackButton) {
        ...
        if ($hasBackButton) {
            Write-Host "  Back button still detected. Pressing back again..." -ForegroundColor Gray
            PressBack
            return $true
        }
    }
    
    # Fallback to GoRead & full check if something is weird
    Write-Host "  EnsureBookList: fallback to full verification..." -ForegroundColor Yellow
    GoRead
    for ($i = 0; $i -lt 4; $i++) {
        $d = GetDump
        $hasBackButton = $d -match "(content-desc|text)=\`"[^\`"]*\u2190[^\`"]*\`""
        $isBookList = $d -match "Holy Bible" -or $d -match "Old Testament" -or $d -match "New Testament"
        if ($isBookList -and -not $hasBackButton) { return $true }
        if ($hasBackButton) {
            PressBack
        } else {
...
function GoToBook([int]$bookNum) {
    if ($script:CurrentBookNum -eq $bookNum) {
        $d = GetDump
        $hasBackButton = $d -match "(content-desc|text)=\`"[^\`"]*\u2190[^\`"]*\`""
        if ($hasBackButton) {
            Write-Host "  Already in book $bookNum reader. Going back to chapter grid..." -ForegroundColor Gray
            PressBack
        }
        return $true
    }
    ...
    # 1. Try finding it directly on the current screen
    $d = GetDump
    $el = FindEl $d $abbr 680 2100
    if ($el.Found) { 
        Tap $el.X $el.Y 1500; 
        $script:CurrentBookNum = $bookNum
        return $true 
    }
    
    # 2. Scroll down to find it
    for ($i = 0; $i -lt 3; $i++) {
        SwipeUp
        $d = GetDump
        $el = FindEl $d $abbr 680 2100
    ...
    # 3. Fallback: scroll to top first
    Write-Host "  Book $abbr not found on current screen. Scrolling to top..." -ForegroundColor Gray
    for ($k = 0; $k -lt 4; $k++) {
        SwipeDown
    }
    
    # 4. Search again, scrolling down if needed
    for ($i = 0; $i -lt 4; $i++) {
        $d = GetDump
        $el = FindEl $d $abbr 680 2100
        if ($el.Found) { 
...
function GoToChapter([int]$bookNum, [int]$chapNum) {
    ...
    # 1. Try finding it directly on current viewport first
    $d = GetDump
    $el = FindEl $d "$chapNum" 340 2200
    ...
    # 3. Fallback: scroll chapter grid to top
    Write-Host "  Chapter $chapNum not found. Scrolling to top..." -ForegroundColor Gray
    for ($k = 0; $k -lt 3; $k++) {
        SwipeDown
    }
...
function Test-FullCoverage {
    ...
    for ($bn = 1; $bn -le 66; $bn++) {
        ...
        if ($ok1) {
            ...
            if ($count1 -gt 0) {
                Write-Host "Ch1:${count1}v " -ForegroundColor Green -NoNewline
...
adb_cmd @("shell","cmd","statusbar","collapse") | Out-Null
adb_cmd @("shell","monkey","-p","$PKG","-c","android.intent.category.LAUNCHER","1") | Out-Null
Start-Sleep -Seconds 25

# Check app is launched
$d = GetDump
$launched = $d -match "Miktam Bible" -or $d -match "Resume Reading" -or $d -match "Verse of the Day" -or $d -match "Old Testament" -or $d -match "New Testament" -or $d -match "Holy Bible"
AssertTest $launched "App launches successfully"
Shot "00_launch"

function EnsureKJV {
    Write-Host "Ensuring KJV version is selected..." -ForegroundColor Cyan
    $script:CurrentBookNum = 0
    GoRead
    Start-Sleep -Seconds 1.5
    
    # 1. Reset to the very beginning of the version picker (scroll all the way to the left)
    Write-Host "Scrolling version picker to the beginning..." -ForegroundColor Gray
    for ($i = 0; $i -lt 4; $i++) {
        adb_cmd @("shell","input","swipe","200","402","900","402","300") | Out-Null
        Start-Sleep -Milliseconds 600
    }
    ...
    for ($scan = 1; $scan -le 6; $scan++) {
        $d = GetDump
        $el = FindEl $d "KJV" 330 470
        if ($el.Found) {
            Write-Host "Found KJV chip at $($el.X) $($el.Y) (scan $scan). Tapping..." -ForegroundColor Green
            Tap $el.X $el.Y 2000
            $found = $true
            break
        }
        
        Write-Host "KJV not found in scan $scan. Scrolling right..." -ForegroundColor Gray
        # Swipe right-to-left to scroll content right
        adb_cmd @("shell","input","swipe","900","402","200","402","500") | Out-Null
        Start-Sleep -Milliseconds 1000
    }
    
    if (-not $found) {
        Write-Host "Failed to find KJV version!" -ForegroundColor Red
    }
}
